...
def get_credential():
    ...
    if not tenant_id or not dataverse_url:
        ...
        print(f"ERROR: .env is missing required values: {', '.join(missing)}", file=sys.stderr, flush=True)
        print("  Run the init sequence (/dataverse:init) to create .env.", file=sys.stderr, flush=True)
        sys.exit(1)
    ...
    try:
        ...
        print("ERROR: azure-identity not installed. Run: pip install --upgrade azure-identity", file=sys.stderr, flush=True)
        sys.exit(1)

    # Warn if only one of CLIENT_ID / CLIENT_SECRET is set
    if bool(client_id) != bool(client_secret):
        print("WARNING: Only one of CLIENT_ID / CLIENT_SECRET is set. Both are required for", file=sys.stderr, flush=True)
        print("  service principal auth. Falling back to interactive device code flow.", file=sys.stderr, flush=True)
    ...
    if client_id and client_secret:
        ...
        def _prompt_callback(verification_uri, user_code, _expires_on):
            print(f"\nTo sign in, visit {verification_uri} and enter code: {user_code}", file=sys.stderr, flush=True)
...
def get_token(scope=None):
    ...
    try:
        ...
        print(f"ERROR: Failed to acquire access token: {e}", file=sys.stderr, flush=True)
        print("  Check your network connection, credentials, and .env configuration.", file=sys.stderr, flush=True)
        sys.exit(1)
